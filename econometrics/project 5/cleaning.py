...
print(data_clean[['proportion_of_urban_population_living_in_slums', 'gdp_per_capita_us_dollars']].head())

# Calculate descriptive statistics for all numeric variables
desc_stats = data_clean[numeric_cols].describe().transpose()

# Add skewness and kurtosis
desc_stats['skewness'] = data_clean[numeric_cols].apply(lambda x: skew(x.dropna()))
desc_stats['kurtosis'] = data_clean[numeric_cols].apply(lambda x: kurtosis(x.dropna()))
...
for var in relevant_vars:
    if var in data_clean.columns:
        ...
        numeric_data = pd.to_numeric(data_clean[var], errors='coerce').dropna()
...
X = data_clean[['gdp_per_capita_us_dollars']]  # Single independent variable
...
print("\nChecking Y variable statistics: ", y.describe())
print("\nChecking X variable statistics: ", X.describe())
